        ...
        if(isValidGrid(map, tempGrid.loc)){//返回1（可行走格）时
          ...
          case 1://
            if(tempGrid.G < pTempGrid->G){//新路线比原open表内格点好
              ...
              // deletLinkListByGrid(openList,pAgainGrid);
            }
            break;
          case 2:
            if(tempGrid.G < pTempGrid->G){// 新路线比原close表内格点好
              ...
              deletLinkListByGrid(openList,pTempGrid);
            } 
            break;
          default:
            break;
...
// LinkGrid* generatePath(Grid *aim){
//   LinkGrid *path = initList(path);
//   do
//   {
